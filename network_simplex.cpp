...
int main(int argv, char** argc) {
    ...
    double graph_plus_algo_time = lemon_graph_time + ns_run_time;
    ...
    // Append runtime stats to CSV (dataset, method, n, flowAmount, total_cost, rotations_num, arc_num, poset_time, algorithm_runtime, graph_plus_algo_time, total_program_time)
    {
        ...
        long cost_out = (ns_result == NetworkSimplex<ListDigraph, int, int>::OPTIMAL) ? (long)ns_cost : -1;
        ofstream csv("runtime_output/network_simplex_runtime_stats.csv", ios::app);
        if (csv.is_open()) {
            if (csv.tellp() == 0) {
            ...
            csv << dataset << ",network_simplex," << n << "," << flowAmount << "," << cost_out
                << "," << n_rotations << "," << arc_num
                << "," << poset_time << "," << ns_run_time << "," << graph_plus_algo_time << "," << total_time << "\n";
            csv.close();
        }
    }
