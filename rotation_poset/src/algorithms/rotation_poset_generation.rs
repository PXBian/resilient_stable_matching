...
pub fn build_rotation_poset(profile: &PreferenceProfile, save_rotations: bool) -> RotationPoset {
    let n = profile.n;

    let mut rp = RotationPoset::new(n);
...
fn grid_update(
    ...
    for &(m, w) in rot.introduced.iter() {
        ...
        rp.rotation_index[m][rm] = rp.n_rotations as isize;
        ...
        for m2 in profile.women[w].ranking.iter().skip(rw + 1) {
            ...
            rp.rotation_index[m2][rm2] = -(rp.n_rotations as isize);
        }
    }
}
