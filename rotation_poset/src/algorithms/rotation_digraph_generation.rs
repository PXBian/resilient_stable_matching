...
pub fn build_rotation_set(profile: &Profile, save_rotations: bool) -> RotationSet {
    let n = profile.n;

    let mut rp = RotationSet::new(n);
...
fn grid_update(
    ...
    for &(m, w) in rot.introduced.iter() {
        ...
        let rm = profile.position_men[m][w] as usize;

        // Mark (m, rm) as stable edge produced by rot_idx
        rp.rotation_index[m][rm] = rp.n_rotations as i32;
        
        // rank of m in w's preferences
        let rw = profile.women_position[w][m] as usize;

        // For all men m' that w ranks lower than m, mark unstable
        for m2 in profile.ranking_women[w].iter().skip(rw + 1) {
            let m2 = *m2 as usize;
            let rm2 = profile.position_men[m2][w] as usize;

            if rp.rotation_index[m2][rm2] != 0 {
                break;
            }

            rp.rotation_index[m2][rm2] = -(rp.n_rotations as i32);
        }
    }
}
...
fn best_next_acceptable_woman(
    ...
    let n = profile.n;
    // rank of w in m's preferences
    let rank = profile.position_men[m][w] as usize;

    // For all men m' that w ranks lower than m, mark unstable
    for i in (rank+1)..n {
        let nw = profile.men_ranking[m][i] as usize;
        if rp.rotation_index[m][i] == 0 {
            return Some(nw);
        }
    }
    None
