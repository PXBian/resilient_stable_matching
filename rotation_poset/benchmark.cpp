...
void run_test(void (*func)(RankingListMatrix, PositionMapMatrix, int), size_t n, unsigned int seed, RankingListMatrix men, PositionMapMatrix women, int complete_data, const std::string& output_file) {
    const std::string& name = (complete_data == 1) ? "complete_info" : "summary";
...
void test_rotation_poset(RankingListMatrix men, PositionMapMatrix women, int complete_data) {
    RotationDigraph digraph = get_rotation_digraph(men, women, complete_data);
    std::cout << "Number of rotations: " << digraph.n_rotations <<", Number of dependencies: " << digraph.n_dependencies << ", Number of pairs: " << digraph.n_pairs << std::endl;
    free_rotation_digraph(digraph);
}

int main(int argc, char* argv[]) {
    if (argc < 5) {
        std::cerr << "Use: " << argv[0] << " <n> <seed> <version> <output_file>" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        run_test(test_rotation_poset, n, seed, men, women, version, output_file);
